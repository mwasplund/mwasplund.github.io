...
## Define
The final way to determine the dependency graph within module interfaces is to simply declare the dependencies for the build system up front. This approach is straight forward and supports any combination of imports and exports.

The challenge with this model is that it is hard to maintain since it often requires repeating information for the build system that is already contained in the source files themselves.

Following the same example as before, imagine a build system that allows you to configure your Source list with a set of Imports to indicate the Translation Unit dependencies which are assumed to map to the Interface Dependencies.
...
Let's walk through the aforementioned example again, this time broken apart into individual projects. Note: This example is a bit contrived as a project will generally contain more than a single source file. Most notable, there will be many implementation and partition units to fill in the details.
